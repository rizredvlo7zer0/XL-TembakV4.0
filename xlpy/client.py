...
class XL(Config):
    ...
    def reqOTP(self):
        ...
        if(len(status) == 3):
            if ("LoginSendOTPRs" in status): return {"message" : b+"Kode Otp Terkirim"}
            else: return {"message" : status['message']}
        if(len(status) == 1): return {"message" : m+"Gagal Meminta Otp"}
    ...
    def purchasePackage(self, serviceid):        
        ...
        status = json.loads(r.content)
        if(len(status) == 4): return {"message" : b+"Paket Berhasil Dibeli"}
        else: return {"message" : status['message']}